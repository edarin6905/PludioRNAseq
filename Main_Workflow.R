...
BiocManager::install(c("limma", "edgeR", "Glimma", "org.Mm.eg.db", "gplots", "RColorBrewer", "NMF", "BiasedUrn"),force=TRUE,dependencies = TRUE)
...
#gene symbols/full gene name. Make annotation
#information in a separate data frame using the select function
...
# how many reads we have for each sample in the y.
...
####Hierarchial Clustoring Heat Maps####
...
aheatmap(highly_variable_lcpm,
         ...
         annCol=sampleinfo[, 3:4],labCol=group, scale="row")

####Normalizing for Computational Bias ####
...
####Differential expression with limmavoom####
...
barcodeplot(fit.cont$coeff[,1], 
            index=c2.ind[["DANG_REGULATED_BY_MYC_UP"]], 
            main="LogFC: DANG_REGULATED_BY_MYC_UP")
...
barcodeplot(fit.cont$t[,1], 
            index=c2.ind[["DANG_REGULATED_BY_MYC_UP"]], 
            main="T-statistic: DANG_REGULATED_BY_MYC_UP")

#Record package version for manuscript
